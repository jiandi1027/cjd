...
public class OverAlarmService  extends CrudService<OverAlarmMapper, OverAlarm> { 
	...
	public void judgeOverAlarm(String x, String y, String mobile, String id, String msg){
		try {
			...
			//拆卸报警	 start
			TerminalHistory terminalHistory = terminalHistoryService.findById(id);
			if (terminalHistory != null) {
				if (terminalHistory.getIsAlarm().equals("拆卸报警")) {
					RemoveAlarm removeAlarm = new RemoveAlarm();
					removeAlarm.setCulpritId(terminalHistory.getCulpritId());
//					removeAlarm.setGpsTime(gpsTime);
					removeAlarm.setTerminalHisId(terminalHistory.getId());
					removeAlarmService.insertSelective(removeAlarm);
				}
			}
			//拆卸报警	 end
		} catch (Exception e) {
