    ...
    /**    
    * @Title: RevokeOutOfPrisonService.java  
...
import com.rowell.sifa.mapper.rewardspunishment.RevokeOutOfPrisonMapper;
...
/**  
    * @ClassName: RevokeOutOfPrisonService  
    * @Description: 提请撤销暂予监外执行service 
    * @author xiaoguang  
    * @date 2017年3月16日  
    *    
    */
...
public class RevokeOutOfPrisonService extends CrudService<RevokeOutOfPrisonMapper, RevokeOutOfPrison> {
	@Autowired
	...
	private static String PROCESS_DEFINITION_KEY = "revokeOutOfPrison";
	@Autowired
	...
	//保存或修改
	public CommonJsonResult saveRevokeOutOfPrison(RevokeOutOfPrison revokeOutOfPrison,List<SysFile> fileList1,List<SysFile> fileList2){
		CommonJsonResult commonJsonResult = null;
		if (StringUtils.isBlank(revokeOutOfPrison.getId())) {
			this.insert(revokeOutOfPrison);
			commonJsonResult = CommonJsonResult.ok();
			commonJsonResult.setData(revokeOutOfPrison.getId());
			commonJsonResult.setMsg("保存成功！");
		} else {
			this.updateByIdSelective(revokeOutOfPrison);
			commonJsonResult = CommonJsonResult.ok();
			commonJsonResult.setData(revokeOutOfPrison.getId());
			commonJsonResult.setMsg("修改成功！");
		}
		sysFileService.saveFiles(fileList1, Constants.FILE_TABLE_REVOKE_OUT_OF_PRISON,revokeOutOfPrison.getId(), Constants.FILE_REVOKE_OUTOFPRISON_ENDCASEPATH);
		sysFileService.saveFiles(fileList2, Constants.FILE_TABLE_REVOKE_OUT_OF_PRISON,revokeOutOfPrison.getId(), Constants.FILE_REVOKE_OUTOFPRISON_NOTEPATH);
	...
	public void submitRevokeOutOfPrison(String id, RevokeOutOfPrison record, ProcessDetail processDetail) {
			...
			//流程启动后在待审批事项内插入数据
			examinationAndApprovalService.insertProcess(record.getId(),record.getCulpritId(),record.getProcessInstanceId(),Constants.PROCESS_TYPE_TQCXJWZX);
	...
	/**
	 * 
	    * @Title: submitRevokeOutOfPrisonAuditStatus  
	    * @Description: 工作流审核审批操作
	    * @param @param roleId
	    * @param @param revokeOutOfPrison
	    * @param @param processDetail    参数  
	    * @return void    返回类型  
	 ...
	 */
	@Transactional(readOnly = false)
	public void submitRevokeOutOfPrisonAuditStatus(String roleId, RevokeOutOfPrison revokeOutOfPrison,ProcessDetail processDetail,List<SysFile> fileList) {
		Task task = taskService.createTaskQuery().taskId(revokeOutOfPrison.getTaskId()).taskAssignee(ProcessUtils.getRoleIdWithGroupId(roleId)).singleResult();
		if (task != null) {
			processDetail.setProcessDefKey(PROCESS_DEFINITION_KEY);
			processDetail.setAuditName(task.getName());
			processDetail.setProcessId(revokeOutOfPrison.getProcessInstanceId());
			...
			Map<String, Object> variables = new HashMap<String, Object>();
			// 根据 auditType判断是几级审核
			if(revokeOutOfPrison.getAuditType().equals("sfsReport")){//如果是司法所工作人员发起，则司法所审核
				variables.put("role_id",  ProcessUtils.getRoleIdWithGroupId(Constants.ROLE_SFSFZR));
				variables.put("decideType", Integer.parseInt(processDetail.getDicDecideType()));
			}else if (revokeOutOfPrison.getAuditType().equals("jzkReport")) {//如果是矫正科工作人员发起，则矫正科审核
				variables.put("role_id",  ProcessUtils.getRoleIdWithGroupId(Constants.ROLE_XJZKFZR));
				variables.put("decideType", Integer.parseInt(processDetail.getDicDecideType()));
			} else if (revokeOutOfPrison.getAuditType().equals("sfsCheck")) {
			...
			} else if (revokeOutOfPrison.getAuditType().equals("jzkCheck")) {
				// 县矫正负责人审核
				...
				if("2".equals(processDetail.getDicDecideType())){//退回操作
					List<ProcessDetail> processDetails=processDetailMapper.findByProcessInsId(revokeOutOfPrison.getProcessInstanceId());
			...
			} else if (revokeOutOfPrison.getAuditType().equals("fzkCheck")) {
				// 县法制科审核
				...
				variables.put("decideType", Integer.parseInt(processDetail.getDicDecideType()));
			} else if (revokeOutOfPrison.getAuditType().equals("xfgCheck")) {
				// 县分管领导 审核
				if("2".equals(processDetail.getDicDecideType())){//退回操作
				...
				variables.put("decideType", Integer.parseInt(processDetail.getDicDecideType()));
			}else if (revokeOutOfPrison.getAuditType().equals("sjzCheck")) {
				// 市矫正负责人审核
				if (!StringUtils.isBlank(processDetail.getOptScore())) {//评分明细
					MxCheck mxCheck = new MxCheck();
					mxCheck.setProcessInstanceId(revokeOutOfPrison.getProcessInstanceId());
				...
				variables.put("decideType", Integer.parseInt(processDetail.getDicDecideType()));
			} else if (revokeOutOfPrison.getAuditType().equals("sfzCheck")) {
				// 市法制科审核
				if (!StringUtils.isBlank(processDetail.getZfScore())) {//评分明细
					MxCheck mxCheck = new MxCheck();
					mxCheck.setProcessInstanceId(revokeOutOfPrison.getProcessInstanceId());
				...
				variables.put("decideType", Integer.parseInt(processDetail.getDicDecideType()));
			} else if (revokeOutOfPrison.getAuditType().equals("sfgCheck")) {
				// 市分管
				...
				variables.put("decideType", Integer.parseInt(processDetail.getDicDecideType()));
			} else if (revokeOutOfPrison.getAuditType().equals("sfsPrint") || revokeOutOfPrison.getAuditType().equals("jzkPrint")) {
				//县、市工作人员打印
				//保存告知材料
				sysFileService.saveFiles(fileList, Constants.FILE_TABLE_REVOKE_OUT_OF_PRISON,revokeOutOfPrison.getId(), Constants.FILE_REVOKE_OUTOFPRISON_GZCLPATH);
				this.updateByIdSelective(revokeOutOfPrison);
			...
			taskService.complete(revokeOutOfPrison.getTaskId(), variables);
