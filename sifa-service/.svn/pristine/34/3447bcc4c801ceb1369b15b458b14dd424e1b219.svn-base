...
import com.rowell.sifa.mapper.endcorrect.RecidivismPerhapsMapper;
...
public class RecidivismPerhapsService extends CrudService<RecidivismPerhapsMapper, RecidivismPerhaps> {
	@Autowired
	...
	private static String PROCESS_DEFINITION_KEY = "recidivismPerhaps";
	...
	public int insert(RecidivismPerhaps record) {
		...
		examinationAndApprovalService.insertProcess(record.getId(),record.getCulpritId(),record.getProcessInstanceId(),Constants.PROCESS_TYPE_SXCXFZ);
		return a;
	}
	 ...
	 */
	@Transactional(readOnly = false)
	public void saveRecidivismPerhapsAuditStatus(String roleId,RecidivismPerhaps recidivismPerhaps,ProcessDetail processDetail) {
		...
		Task task = taskService.createTaskQuery().taskId(recidivismPerhaps.getTaskId()).taskAssignee(ProcessUtils.getRoleIdWithGroupId(roleId)).singleResult();
		...
		if (task != null) {
			...
			processDetail.setProcessId(recidivismPerhaps.getProcessInstanceId());
			...
			if(recidivismPerhaps.getAuditType().equals("workerReport")){
			...
			}else if (recidivismPerhaps.getAuditType().equals("sfsCheck")) {
				//
			...
			} else if (recidivismPerhaps.getAuditType().equals("jzkCheck")) {
				// 县矫正负责人审核
				if("2".equals(processDetail.getDicDecideType())){//退回操作
					variables.put("role_id",ProcessUtils.getRoleIdWithGroupId( Constants.ROLE_SFSFZR));
				}else{
				...
				variables.put("decideType", Integer.parseInt(processDetail.getDicDecideType()));
			} else if (recidivismPerhaps.getAuditType().equals("xfgCheck")) {
				// 县分管领导 审核
			...
			taskService.complete(recidivismPerhaps.getTaskId(), variables);
