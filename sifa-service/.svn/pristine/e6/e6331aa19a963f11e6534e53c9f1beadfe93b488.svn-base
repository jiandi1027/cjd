...
public class AskLeaveService extends CrudService<AskLeaveMapper, AskLeave> {
	...
	public int insertMore(AskLeave record, List<SysFile> fileList) {
		...
		sysFileService.saveFiles(fileList, Constants.FILE_TABLE_ASKLEAVE, record.getId(), Constants.FILE_ASKLEAVE_FILE);
		examinationAndApprovalService.insertProcess(record.getId(), record.getCulpritId(),
				record.getProcessInstanceId(), Constants.PROCESS_TYPE_QXJGL);
		
		return result;
	}

	@Transactional(readOnly = false)
	public int updateMore(AskLeave record, List<SysFile> fileList) {
	...
	public void saveAskLeaveAuditStatus(String roleId, AskLeave askLeave,ProcessDetail processDetail) {
		...
		if (task != null) {
			...
			if(askLeave.getAuditType().equals("workerPrint")||(askLeave.getAuditType().equals("sfsCheck")&&askLeave.getLeaveDays()<=7)){//流程走完 变成未销假
				...
				if (culprit != null) {
					culprit.setDicStatusKey(Constants.CULPRIT_STATUS_WCQJ);
