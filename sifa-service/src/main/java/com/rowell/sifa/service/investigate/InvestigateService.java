...
public class InvestigateService extends CrudService<InvestigateMapper, Investigate> {
	...
	@Transactional(readOnly = false)
	public int insertMore(Investigate record, List<SysFile> fileList) {
		...
		sysFileService.saveFiles(fileList, Constants.FILE_TABLE_INVESTIGATE, record.getId(), Constants.FILE_INVESTIGATE_WTDCH_PATH);
		examinationAndApprovalService.insertProcess(record.getId(), null, record.getProcessInstanceId(), Constants.PROCESS_TYPE_SQDC);
		return result;
	}
	
	@Transactional(readOnly = false)
	public int updateMore(Investigate record, List<SysFile> fileList, List<SysFile> basicViewfile, List<SysFile> policeViewfile, List<SysFile> preRecordfile){

		sysFileService.saveFiles(fileList, Constants.FILE_TABLE_INVESTIGATE, record.getId(), Constants.FILE_INVESTIGATE_WTDCH_PATH);
		sysFileService.saveFiles(basicViewfile, Constants.FILE_TABLE_INVESTIGATE, record.getId(), Constants.FILE_INVESTIGATE_BASICVIEW);
		sysFileService.saveFiles(policeViewfile, Constants.FILE_TABLE_INVESTIGATE, record.getId(), Constants.FILE_INVESTIGATE_POLOCEVIEW);
		sysFileService.saveFiles(preRecordfile, Constants.FILE_TABLE_INVESTIGATE, record.getId(), Constants.FILE_INVESTIGATE_PRERECORD);
		
		return dao.updateByPrimaryKeySelective(record);
	}
	
	@Transactional(readOnly = false)
	public void saveInvestigateAuditStatus(String roleId, Investigate investigate, String update, ProcessDetail processDetail, 
			List<SysFile> basicViewfile, List<SysFile> policeViewfile, List<SysFile> preRecordfile) {
		...
		if (task != null) {
			...
			if (!StringUtils.isBlank(update)) {
				sysFileService.saveFiles(basicViewfile, Constants.FILE_TABLE_INVESTIGATE, investigate.getId(),
						Constants.FILE_INVESTIGATE_BASICVIEW);
				sysFileService.saveFiles(policeViewfile, Constants.FILE_TABLE_INVESTIGATE, investigate.getId(),
						Constants.FILE_INVESTIGATE_POLOCEVIEW);
				sysFileService.saveFiles(preRecordfile, Constants.FILE_TABLE_INVESTIGATE, investigate.getId(),
						Constants.FILE_INVESTIGATE_PRERECORD);
				updateMore(investigate, null, basicViewfile, policeViewfile, preRecordfile);
			}
