...
import com.rowell.sifa.mapper.dailymgr.OutOfPrisonMapper;
...
public class OutOfPrisonService extends CrudService<OutOfPrisonMapper, OutOfPrison> {
	@Autowired
	...
	private static String PROCESS_DEFINITION_KEY = "outOfPrison";
	@Autowired
	...
	public CommonJsonResult saveOutOfPrison(OutOfPrison outOfPrison,List<SysFile> fileList1,List<SysFile> fileList2){
		CommonJsonResult commonJsonResult = null;
		if (StringUtils.isBlank(outOfPrison.getId())) {
			this.insert(outOfPrison);
			commonJsonResult = CommonJsonResult.ok();
			commonJsonResult.setData(outOfPrison.getId());
			commonJsonResult.setMsg("保存成功");
		} else {
			this.updateByIdSelective(outOfPrison);
			commonJsonResult = CommonJsonResult.ok();
			commonJsonResult.setData(outOfPrison.getId());
			commonJsonResult.setMsg("修改成功");
		}
		sysFileService.saveFiles(fileList1, Constants.FILE_TABLE_OUT_OF_PRISON,outOfPrison.getId(), Constants.FILE_OUTOFPRISON_ENDCASEPATH);
		sysFileService.saveFiles(fileList2, Constants.FILE_TABLE_OUT_OF_PRISON,outOfPrison.getId(), Constants.FILE_OUTOFPRISON_NOTEPATH);
	...
	public void submitOutOfPrison(String id, OutOfPrison record, ProcessDetail processDetail) {
		...
		//流程启动后在待审批事项内插入数据
		examinationAndApprovalService.insertProcess(record.getId(),record.getCulpritId(),record.getProcessInstanceId(),Constants.PROCESS_TYPE_YCJWZX);
		int result = updateById(record);	
	}
	
	/**
	 * 
	    * @Title: submitOutOfPrisonAuditStatus  
	    * @Description: 流程处理
	    * @param @param roleId
	 ...
	 */
	@Transactional(readOnly = false)
	public void submitOutOfPrisonAuditStatus(String roleId, OutOfPrison outOfPrison, ProcessDetail processDetail,List<SysFile> fileList) {
		Task task = taskService.createTaskQuery().taskId(outOfPrison.getTaskId()).taskAssignee(ProcessUtils.getRoleIdWithGroupId(roleId)).singleResult();
		if (task != null) {
			processDetail.setAuditName(task.getName());
			processDetail.setProcessId(outOfPrison.getProcessInstanceId());
			...
			Map<String, Object> variables = new HashMap<String, Object>();
			// 根据 auditType判断是几级审核
			if(outOfPrison.getAuditType().equals("sfsReport")){//如果是司法所工作人员发起，则司法所审核
				variables.put("role_id", ProcessUtils.getRoleIdWithGroupId(Constants.ROLE_SFSFZR));
				variables.put("decideType", Integer.parseInt(processDetail.getDicDecideType()));
			}else if (outOfPrison.getAuditType().equals("jzkReport")) {//如果是矫正科工作人员发起，则矫正科审核
				variables.put("role_id", ProcessUtils.getRoleIdWithGroupId(Constants.ROLE_XJZKFZR));
				variables.put("decideType", Integer.parseInt(processDetail.getDicDecideType()));
			} else if (outOfPrison.getAuditType().equals("sfsCheck")) {
			...
			} else if (outOfPrison.getAuditType().equals("jzkCheck")) {
				// 县矫正负责人审核
				...
				if("2".equals(processDetail.getDicDecideType())){//退回操作
					...
					if(outOfPrison.getIsConform()==1 && outOfPrison.getIsDeadline()==0){
						variables.put("role_id", ProcessUtils.getRoleIdWithGroupId(Constants.ROLE_XFGLD));//县分管领导
						variables.put("CD", 10);
					}else{
			...
			} else if (outOfPrison.getAuditType().equals("fzkCheck")) {
				// 县法制科审核
				...
				variables.put("decideType", Integer.parseInt(processDetail.getDicDecideType()));
			} else if (outOfPrison.getAuditType().equals("xfgCheck")) {
				// 县分管领导 审核
				if("2".equals(processDetail.getDicDecideType())){//退回操作
					List<ProcessDetail> processDetails=processDetailService.findByProcessInsId(outOfPrison.getProcessInstanceId());
					for(ProcessDetail p :processDetails){
						System.out.println(p.getAuditName());
						if("县法制科审核".equals(p.getAuditName())){//退回县法制科		
					...
					if("91302".equals(outOfPrison.getDicSubOrgTypeKey())){//呈报市局，流程至市局矫正科
						variables.put("role_id", ProcessUtils.getRoleIdWithGroupId(Constants.ROLE_SJZCFZR));
						variables.put("decideType", 6);
					}else{//不呈报市局
						variables.put("role_id", ProcessUtils.getRoleIdWithGroupId(Constants.ROLE_XJZKGZRY));
			...
			}else if (outOfPrison.getAuditType().equals("sjzCheck")) {
				// 市矫正负责人审核
				if (!StringUtils.isBlank(processDetail.getOptScore())) {//评分明细
					MxCheck mxCheck = new MxCheck();
					mxCheck.setProcessInstanceId(outOfPrison.getProcessInstanceId());
				...
				variables.put("decideType", Integer.parseInt(processDetail.getDicDecideType()));
			} else if (outOfPrison.getAuditType().equals("sfzCheck")) {
				// 市法制科审核
				if (!StringUtils.isBlank(processDetail.getZfScore())) {//评分明细
					MxCheck mxCheck = new MxCheck();
					mxCheck.setProcessInstanceId(outOfPrison.getProcessInstanceId());
				...
				variables.put("decideType", Integer.parseInt(processDetail.getDicDecideType()));
			} else if (outOfPrison.getAuditType().equals("sfgCheck")) {
				// 市分管
				...
				variables.put("decideType", Integer.parseInt(processDetail.getDicDecideType()));
			} else if (outOfPrison.getAuditType().equals("sfsPrint") || outOfPrison.getAuditType().equals("jzkPrint")) {
				//县、市工作人员打印
				//保存告知材料
				sysFileService.saveFiles(fileList, Constants.FILE_TABLE_OUT_OF_PRISON,outOfPrison.getId(), Constants.FILE_OUTOFPRISON_GZCLPATH);
				this.updateByIdSelective(outOfPrison);	
			}
			// 提交审核时，设置流程变量，变量值就是审核 信息
			taskService.complete(outOfPrison.getTaskId(), variables);
		}
