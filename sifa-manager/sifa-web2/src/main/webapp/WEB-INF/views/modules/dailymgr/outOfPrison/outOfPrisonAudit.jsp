	 ...
	 <div class="easyui-panel" id="outOfPrison_p1" title="申请相关信息" style="width:100%;padding:5px;margin:0 auto;">
            <table width="100%" class="grid">
                ...
                <c:if test="${outOfPrison.auditType eq 'sfsReport' || outOfPrison.auditType eq 'jzkReport'}">
                 <tr>
	                 <th width="10%">编辑建议书：</th>
	                 <td> 
	                 	<a onclick="printOutOfPrisonBook('${outOfPrison.id}');" href="javascript:void(0);" class="easyui-linkbutton" data-options="iconCls:'fi-pencil icon-blue'">编辑</a>
	                 </td>
	             </tr>
	             </c:if>
	            <!--  <tr>
...
<div class="easyui-panel" title="审批信息" style="width:100%;padding:5px;margin:0 auto;">
            <table width="100%" class="grid">
            	<TR>	
					...
					<Td style="border:0;" width="10%"></Td ><TD width="23%" style="border:0;"></TD>
				</TR>
				...
				<TR>	
					...
					<TD width="23%">
						<input class="easyui-textbox"  name="submittionOrg" />
					</TD>
					<Th width="10%" >抄送单位名称：</Th>
					<TD width="23%">
						<input class="easyui-textbox"  name="copyOrg" />
					</TD>
				</TR>		    
				</c:if>
...
<div id="outOfPrisonrewardspunishmentForm"></div>
<script type="text/javascript">
var adtype = '${outOfPrison.auditType}';
//编辑建议表
function printOutOfPrisonBook(id){
	$('#printJiashi').dialog({    
	    title: '编辑《延长暂予监外执行建议书》',    
	    width: 900,    
	    height: 800,    
	    closed: false,    
	    cache: false,
	    ...
	    href: '${ctx}/dailymgr/outOfPrison/preSubmit?id='+id+"&audit=1",    
	    modal: true,
	    buttons :[{
			text:'保存',
            iconCls:'fi-print icon-blue',
			handler:function(){
				$('#saveOutOfPrisonBookBtn').click();
			}
		}, {
			text:'关闭',
...
var result='';
 $(function() {
	...
	$('#outOfPrisonAuditForm').form({
		url:'${ctx}/dailymgr/outOfPrison/submitOutOfPrisonAudit',
		onSubmit:function(){
			 progressLoad();
			 var adtypemsg = '';
...
//评分
 function rewardspunishment(type){
 		var url = '${ctx}/sys/mxCheck/assess?infoType='+ type +'&processInstanceId=${outOfPrison.processInstanceId}&tablename=outOfPrisonrewardspunishmentForm';
 		var title = "评定明细";
 		...
 		$('#outOfPrisonrewardspunishmentForm').dialog({    
 		    title: title,    
 		    width: 600,
 		    height: 520,
 		    href: url,    
 		...
 		$('#outOfPrisonrewardspunishmentForm').window('center');
 	}
 	...
 	$('#outOfPrisonrewardspunishmentForm').dialog({    
        ...
        maximizable:true,
 	    buttons :[{
 			...
 			iconCls:'fi-x icon-blue',
 			handler:function(){
 				$('#outOfPrisonrewardspunishmentForm').dialog('close');
 			}
 		}]
 	});
 	$('#outOfPrisonrewardspunishmentForm').window('center');
