...
<form action="${ctx}/monitoring/locationhis/detail" method="post" id="locationhisDetailForm">
	...
	<div class="easyui-panel" id="p2" title="" style="width: 100%; padding: 5px;margin: 0 auto;">
		<table width="100%" class="grid">
			<tr>
				<th width="10%">矫正对象：</th>
				<td width="40%">${terminalActivity.culpritName }</td>
				<th width="10%">定位号码：</th>
				<td width="40%">${terminalActivity.mobileNumber }</td>
			</tr>
			<tr>
				<th width="10%">定位方式：</th>
				<td width="40%">${fns:getSysDicValueByKey('SF_TERMINAL_ACTIVITY', 'location_type', terminalActivity.locationType, '') }</td>
				<th width="10%">定位时间：</th>
				<td width="40%"><fmt:formatDate value="${terminalActivity.gpstime}" pattern="yyyy-MM-dd hh:mm:ss" /></td>
			</tr>
			<tr>
				<th width="10%">定位经度：</th>
				<td width="40%">${terminalActivity.lon }</td>
				<th width="10%">定位纬度：</th>
				<td width="40%">${terminalActivity.lat }</td>
			</tr>
			<tr>
				<th width="10%" rowspan="2">定位位置：</th>
				<td width="90%" colspan="3">${terminalActivity.location }
			</tr>
			<tr height="200px;">
...
var point1 = new AMap.LngLat('${terminalActivity.lon }','${terminalActivity.lat }');
