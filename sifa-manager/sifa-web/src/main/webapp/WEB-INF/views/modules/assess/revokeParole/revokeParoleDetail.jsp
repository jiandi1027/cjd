...
<body>
     ...
     <div class="easyui-panel" id="p2" title="撤销缓刑相关信息" style="width:100%;padding:10px;">
            <table width="100%">
                <tr>
					<td width="20%">违纪原因：</td>
					<td width="63%">${fns:getSysDicValueByKey('sf_revoke_probation','REASON_TYPE_ID',revokeParole.dicReasonTypeKey,'')}</td>
                </tr>
                <tr>
                    <td>结案报告：</td>
                    <td>${revokeParole.endCasePath }</td>
                </tr>
                <tr>
                    <td>证据材料：</td>
                    <td>${revokeParole.notePath }</td>
                </tr>
                <tr>
                    <td>简要违纪情况：</td><td>${revokeParole.detailInfo }</td>
                </tr>
                <tr>
                    <td>处理依据：</td><td>${revokeParole.fact }</td>
                </tr>
            </table>
    </div>
    ...
    <div align="center">
		<a href="javascript:void(0)" class="easyui-linkbutton"  onclick="preSubmit('+${revokeParole.id}+');">打印《撤销假释建议书》</a>
		&nbsp;&nbsp;&nbsp; &nbsp;&nbsp;&nbsp; &nbsp;&nbsp;&nbsp;
  		<a href="javascript:void(0)" class="easyui-linkbutton"  onclick="printJianYi('+${revokeParole.id}+');">打印《撤销假释审批表》</a>
  		&nbsp;&nbsp;&nbsp; &nbsp;&nbsp;&nbsp; &nbsp;&nbsp;&nbsp;
...
function preSubmit(id){
	var url = '${ctx}/jiangcheng/revokeParole/preSubmit?detail=1&&id=' + id;//detail标记为详细页面，建议书不能修改
	open_layer('打印《撤销假释建议书》',url);
}

//打印《撤销缓刑审批表》
function printJianYi(id){
	var url = '${ctx}/jiangcheng/revokeParole/printShenhe?id=' + id;
	open_layer('打印《撤销假释审批表》',url);
}
