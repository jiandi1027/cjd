...
<body>
     ...
     <div class="easyui-panel" id="p2" title="撤销缓刑相关信息" style="width:100%;padding:10px;">
            <table width="100%">
                <tr>
					<td>违纪原因：</td><td>${fns:getSysDicValueByKey('sf_revoke_probation','REASON_TYPE_ID',revokeProbation.reasonTypeId,'')}</td>
                </tr>
                <tr>
                    <td>结案报告：</td><td>${revokeProbation.endCasePath }</td>
                </tr>
                <tr>
                    <td>证据材料：</td><td>${revokeProbation.notePath }</td>
                </tr>
                <tr>
                    <td>简要违纪情况：</td><td>${revokeProbation.detailInfo }</td>
                </tr>
                <tr>
                    <td>处理依据：</td><td>${revokeProbation.fact }</td>
                </tr>
            </table>
    </div>
    ...
    <div align="center">
		<a href="javascript:void(0)" class="easyui-linkbutton"  onclick="preSubmit('+${revokeProbation.id}+');">打印《撤销缓刑建议书表》</a>
		&nbsp;&nbsp;&nbsp; &nbsp;&nbsp;&nbsp; &nbsp;&nbsp;&nbsp;
  		<a href="javascript:void(0)" class="easyui-linkbutton"  onclick="printJianYi('+${revokeProbation.id}+');">打印《撤销缓刑审批表》</a>
  		&nbsp;&nbsp;&nbsp; &nbsp;&nbsp;&nbsp; &nbsp;&nbsp;&nbsp;
...
//打印《撤销缓刑审批表》
function printJianYi(id){
	var url = '${ctx}/jiangcheng/revokeProbation/printJianYi?id=' + id;
	open_layer('打印《撤销缓刑审批表》',url);
}
//打印《撤销缓刑建议书》
function preSubmit(id){
	var url = '${ctx}/jiangcheng/revokeProbation/preSubmit?detail=1&&id=' + id;
	open_layer('打印《撤销缓刑建议书》',url);
}
