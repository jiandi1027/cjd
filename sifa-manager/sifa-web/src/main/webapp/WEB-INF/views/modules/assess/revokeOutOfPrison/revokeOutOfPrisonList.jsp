...
<body class="easyui-layout" id="lay">
	...
	<div region="center" border="false">
		<table class="easyui-datagrid" id="t_list_content" title="提请撤销暂予监外执行"
			style="height: 400px;"
			data-options="singleSelect:true,fit:true,rownumbers:true,collapsible:true,striped:true,toolbar:'#toolbar',loadMsg: '数据正在加载,请耐心的等待...' ,pagination:true,pageSize: 10 ,pageList:[5,10,15,20,50],fitColumns:true,url:'${ctx}/jiangcheng/revokeOutOfPrison/list',method:'post'">
			<thead>
				<tr>
	...
	<div id="toolbar" style="display: none;">
		...
		<a onclick="open_layer('新增提请撤销监外执行','${ctx}/jiangcheng/revokeOutOfPrison/form');" class="easyui-linkbutton" data-options="plain:true,iconCls:'icon-add'" id="addbtn">新增</a>
		</c:if>
...
function deleteFun() {
	...
	if (arr.length <= 0) {
		...
		$.messager.confirm('提示信息', '确认删除?', function(r) {
			if (r) {
				$.post('${ctx}/jiangcheng/revokeOutOfPrison/delete', {
					id : arr[0].id
				}, function(result) {
					//1 刷新数据表格 
...
function formatOper(val,row,index){  
	...
	if(row.hasPermission==1){
		operation=operation+'&nbsp;&nbsp;&nbsp;<a href="javascript:void(0);" onclick="submitAuditRevokeOutOfPrison(\''
...
function viewProcess(url){
	//console.log(url);
	open_layer('撤销暂予监外执行流程',url);
}
//流程审批
function submitAuditRevokeOutOfPrison(taskId,id,auditType,processId){
	open_layer("撤销暂予监外执行审核审批","${ctx}/jiangcheng/revokeOutOfPrison/revokeOutOfPrisonAudit?id=" + id+"&taskId="+taskId+"&auditType="+auditType+"&processInstanceId="+processId);
}
//详细
function detail(id){
	var url = '${ctx}/jiangcheng/revokeOutOfPrison/detail?id=' + id;
	open_layer('撤销暂予监外执行详情',url);
}
//上报修改页面
function edit(id){
	open_layer('撤销暂予监外执行（上报/修改）','${ctx}/jiangcheng/revokeOutOfPrison/form?id='+id);
}
