...
<title>撤销暂予监外执行审核</title>

</head>
<body>

<form id="revokeOutOfPrisonAuditForm" name="revokeOutOfPrisonAuditForm" action="${ctx}/jiangcheng/revokeOutOfPrison/submitRevokeOutOfPrisonAudit" method="post">
			...
			<TR>
				<TD>
					<TABLE class="toptable grid" border=1 cellSpacing=1 cellPadding=4
						...
						<TBODY>
							<tr>
								<td height=30 width="15%" align=left>违纪原因</td><td>${fns:getSysDicValueByKey('sf_revoke_probation','REASON_TYPE_ID',revokeOutOfPrison.dicReasonTypeKey,'')}</td>
				            </tr>
				            <tr>
				                 <td height=30 width="15%" align=left>结案报告:</td><td>${revokeOutOfPrison.endCasePathName }</td>
				            </tr>
				             <tr>
				                 <td height=30 width="15%" align=left>证据材料:</td><td>${revokeOutOfPrison.notePathName }</td>
				             </tr> 
				             <tr>
			                    <td height=30 width="15%" align=left>简要违纪情况：</td><td>${revokeOutOfPrison.detailInfo }</td>
			                </tr>
			                <tr>
			                    <td height=30 width="15%" align=left>处理依据：</td><td>${revokeOutOfPrison.fact }</td>
			                </tr>
							</TBODY>
						</TABLE>
					</TD>
...
<TABLE border=0 cellSpacing=0 cellPadding=0 width="100%" bgColor=#c4d8ed>
							  ...
							  && revokeOutOfPrison.auditType ne 'jzkPrint' && revokeOutOfPrison.auditType ne 'sfsPrint'}">
							   <TR>	
				                ...
				                <c:if test="${revokeOutOfPrison.auditType ne 'sfsReport' && revokeOutOfPrison.auditType ne 'jzkReport' 
									...
									&& revokeOutOfPrison.auditType ne 'jzkPrint' && revokeOutOfPrison.auditType ne 'sfsPrint'}">
			                 	<%-- <TR>	
									<c:if test="${revokeOutOfPrison.auditType eq 'jzkCheck'||revokeOutOfPrison.auditType eq 'sjzCheck'}">
								...
								<TR>	
									...
									<TD >
										<select style="width:150px;" editable=false name="dicSubOrgTypeKey" required=true missingMessage="呈报不能为空!" value="${revokeOutOfPrison.dicSubOrgTypeKey }"
 ...
 <TABLE border=0 cellSpacing=0 cellPadding=0 width="100%" bgColor=#c4d8ed>
  <TBODY>
  	<c:if test="${revokeOutOfPrison.auditType ne 'sfsPrint'&& revokeOutOfPrison.auditType ne 'jzkPrint'}">
 	<tr>
	...
	<c:if test="${revokeOutOfPrison.auditType eq 'sfsPrint' || revokeOutOfPrison.auditType eq 'jzkPrint'}">
 	<tr>
  		<td colspan=4 align=center class=category>
...
//编辑撤销缓刑建议表
function printrevokeOutOfPrisonBook(){
	 var id=$("#revokeOutOfPrisonId").val();
	 var url = '${ctx}/jiangcheng/revokeOutOfPrison/preSubmit?id=' + id;
...
//提交
function doAudit(){
	 if($("#revokeOutOfPrisonAuditForm").form('validate')){
		 $.ajax({
			type: 'post' ,
			url: $("#revokeOutOfPrisonAuditForm").attr("action"),
			cache:false ,
			data:$('#revokeOutOfPrisonAuditForm').serialize() ,
			dataType:'json',
...
/* $(function() {
	$('#revokeOutOfPrisonAuditForm').form({
		url:'${ctx}/jiangcheng/revokeOutOfPrison/submitrevokeOutOfPrisonAudit',
		onSubmit:function(){
