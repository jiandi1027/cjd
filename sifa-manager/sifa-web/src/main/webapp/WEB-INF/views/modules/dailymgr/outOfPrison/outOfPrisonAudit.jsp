...
<form id="outOfPrisonAuditForm" name="outOfPrisonAuditForm" action="${ctx}/dailymgr/outOfPrison/submitOutOfPrisonAudit" method="post">
	<input type="hidden" name="taskId" value="${outOfPrison.taskId}"/>
	<input type="hidden" name="id" id="outOfPrisonId" value="${outOfPrison.id}"/>
			...
			<TR>
				<TD>
					<TABLE class="toptable grid" border=1 cellSpacing=1 cellPadding=4
						...
						<TBODY>
							<tr>
								<td height=40 width="15%" align=left>申请延长原因及依据：</td><td>${outOfPrison.reason }</td>
				            </tr>
				            <tr>
				                 <td height=20 width="15%" align=left>延长期限（月）：</td><td>${outOfPrison.extendTime }</td>
				            </tr>
				             <tr>
				                 <td height=20 width="15%" align=left>具体延长期限起止：</td><td><fmt:formatDate value="${outOfPrison.startDate }"
									pattern="yyyy年MM月dd日" />至
									<fmt:formatDate value="${outOfPrison.endDate }"
									pattern="yyyy年MM月dd日" /></td>
				            </tr>
				            <tr>
				                 <td height=20 width="15%" align=left>鉴定结果是否符合条件：</td>
				                 <td><c:if test="${outOfPrison.isConform  eq '1' }">是</c:if>
				                 	<c:if test="${outOfPrison.isConform  eq '0' }">否</c:if>
				                 </td>
				            </tr>
				            <tr>
				                 <td height=20 width="15%" align=left>有无明确期限:</td>
				                 <td><c:if test="${outOfPrison.isDeadline  eq '1' }">有</c:if>
				                 	<c:if test="${outOfPrison.isDeadline  eq '0' }">无</c:if>
				                 </td>
				            </tr>
				             <tr>
				                 <td height=20 width="15%" align=left>已上传法律文书附件:</td><td>${outOfPrison.lawNotePathName }</td>
				             </tr>
				              <tr>
				                 <td height=20 width="15%" align=left>已上传证据材料附件:</td><td>${outOfPrison.notePathName }</td>
				             </tr>
							</TBODY>
						</TABLE>
					</TD>
...
<c:if test="${outOfPrison.auditType ne 'sfsReport' && outOfPrison.auditType ne 'jzkReport' 
	&& outOfPrison.auditType ne 'sfsCheck'}">
<TABLE border=0 cellSpacing=0 cellPadding=0 width="100%" bgColor=#c4d8ed>
   <TBODY>
   <TR>
				<TD background="${ctxStatic}/images/r_0.gif" width="100%">
...
<TABLE border=0 cellSpacing=0 cellPadding=0 width="100%" bgColor=#c4d8ed>
   ...
   <TR>
			...
			<TR>
				<TD>
					<TABLE class="toptable grid" border=1 cellSpacing=1 cellPadding=4 bgColor=#c4d8ed
						...
						<TBODY>
							 <c:if test="${outOfPrison.auditType ne 'sfsReport' && outOfPrison.auditType ne 'jzkReport'
							  && outOfPrison.auditType ne 'jzkPrint' && outOfPrison.auditType ne 'sfsPrint'}">
							   <TR>	
									<TD height=30 width="15%" align=left >审核结果：</TD>
									<TD  width="35%">
										<input type="radio" name="decideType" checked="checked" value="1"/>同意
										<c:if test="${outOfPrison.auditType eq 'sfgCheck' }"><input type="radio" name="decideType" value="0"/>不同意</c:if>
				                ...
				                <c:if test="${outOfPrison.auditType ne 'sfsReport' && outOfPrison.auditType ne 'jzkReport' 
									...
									&& outOfPrison.auditType ne 'jzkPrint' && outOfPrison.auditType ne 'sfsPrint'}">
			                 	<TR>	
									<TD height=30 align=left >延长暂予监外执行建议书：</TD>
									<TD >
										<input type="button"  value="编辑" onclick="printRevokeParoleBook();"/>
									</TD>
									<c:if test="${outOfPrison.auditType eq 'jzkCheck'||outOfPrison.auditType eq 'sjzCheck'}">
									...
									<td width="35%">
										<input class="easyui-validatebox" required="true" missingMessage="质量评分不能为空!" readonly="readonly" id="handleQualityGoal" name="optScore">
									</td>
								</tr></c:if>
								</TR>	
								<c:if test="${outOfPrison.auditType eq 'jzkCheck'}">		    
								...
								<TR>	
									<TD height=30 align=left >是否报市局：</TD>
									<TD>
										<input type="radio" name="dicSubOrgTypeKey" value="91301" />不报市局
										<input type="radio" name="dicSubOrgTypeKey" value="91302" checked="checked"/>呈报市局
									</TD>
								</TR>
								<TR>	
								...
								<c:if test="${outOfPrison.auditType eq 'fzkCheck'}">		    
								<TR>	
									<TD height=30 align=left >执法质量：</TD>
									<TD width="35%">
										<input type="button"  value="评定" onclick="$('#tt').window('open')" />
								...
								<c:if test="${outOfPrison.auditType eq 'sjzCheck'}">		    
								<TR>	
									<TD height=30 align=left >执法质量：</TD>
									<TD width="35%">
										<input type="button"  value="评定" onclick="$('#tt').window('open')" />
								...
								<c:if test="${outOfPrison.auditType eq 'sfzCheck'}">		    
								<TR>	
									<TD height=30 align=left >执法质量：</TD>
									<TD width="35%">
										<input type="button"  value="评定" onclick="$('#tt').window('open')" />
 	 ...
 	 <c:if test="${outOfPrison.auditType eq 'sfsReport'|| outOfPrison.auditType eq 'jzkReport'
 	 			|| outOfPrison.auditType eq 'sfsPrint' || outOfPrison.auditType eq 'jzkPrint'}">
	 <input type="hidden" name="decideType" value="1"/>
	 </c:if>
 <TABLE border=0 cellSpacing=0 cellPadding=0 width="100%" bgColor=#c4d8ed>
  <TBODY>
  	<c:if test="${outOfPrison.auditType ne 'sfsPrint'&& outOfPrison.auditType ne 'jzkPrint'}">
 	<tr>
	...
	<c:if test="${outOfPrison.auditType eq 'sfsPrint' || outOfPrison.auditType eq 'jzkPrint'}">
 	<tr>
  		<td colspan=4 align=center class=category>
...
//提交
function doAudit(){
	 if($("#outOfPrisonAuditForm").form('validate')){
		 $.ajax({
			type: 'post' ,
			url: $("#outOfPrisonAuditForm").attr("action"),
			cache:false ,
			data:$('#outOfPrisonAuditForm').serialize() ,
			dataType:'json',
