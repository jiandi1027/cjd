...
<body class="easyui-layout" id="lay">
	...
	<div region="center" border="false">
		<table class="easyui-datagrid" id="t_list_content" title="口头警告列表"
			style="height: 400px;"
			data-options="singleSelect:true,collapsible:true,striped:true,toolbar:toolbar,loadMsg: '数据正在加载,请耐心的等待...' ,pagination:true,pageSize: 10 ,pageList:[5,10,15,20,50],fitColumns:true,url:'${ctx}/jiangcheng/verbalWarning/list',method:'post'">
			<thead>
				<tr>
...
<script type="text/javascript">
	...
	var toolbar = [ {
		...
		handler : function() {
			...
			open_layer('新增口头警告','${ctx}/jiangcheng/verbalWarning/add');
	...
	function viewProcess(url){
		open_layer('口头警告流程',url);
	}
	function submitAudit(id){
		open_layer("口头警告审核审批","${ctx}/jiangcheng/verbalWarning/check?id=" + id);
	}
	function detail(id){
		var url = '${ctx}/jiangcheng/verbalWarning/get?id=' + id;
		open_layer('口头警告流程详情',url);
		/* $.ajax({
			type : 'get',
