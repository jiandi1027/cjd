...
<body class="easyui-layout" id="lay">
	...
	<div region="center" border="false">
		<table class="easyui-datagrid" id="t_list_content" title="提请撤销假释列表"
			style="height: 400px;"
			data-options="singleSelect:true,fit:true,rownumbers:true,collapsible:true,striped:true,toolbar:'#toolbar',loadMsg: '数据正在加载,请耐心的等待...' ,pagination:true,pageSize: 10 ,pageList:[5,10,15,20,50],fitColumns:true,url:'${ctx}/jiangcheng/revokeParole/list',method:'post'">
			<thead>
				<tr>
					...
					<th data-options="field:'updated',width:120,align:'center',sortable:true"
						editor="text">更新时间</th>
					<th data-options="field:'dicReasonTypeKey',width:200,align:'center',formatter:formatStatusKey"
	...
	<div id="toolbar" style="display: none;">
		...
		<a onclick="open_layer('新增提请撤销假释','${ctx}/jiangcheng/revokeParole/form');" class="easyui-linkbutton" data-options="plain:true,iconCls:'icon-add'" id="addbtn">新增</a>
		</c:if>
...
function deleteFun() {
	...
	if (arr.length <= 0) {
		...
		$.messager.confirm('提示信息', '确认删除?', function(r) {
			if (r) {
				$.post('${ctx}/jiangcheng/revokeParole/delete', {
					id : arr[0].id
				}, function(result) {
					//1 刷新数据表格 
...
function formatOper(val,row,index){  
	var operation=$.formatString('<a href="javascript:revokeParole(\'{0}\');"  title="详细"><span style="color:blue;"><strong><u>详细</u><strong></span></a>', row.id);;
	...
	if(row.hasPermission==1){
		operation=operation+'&nbsp;&nbsp;&nbsp;<a href="javascript:void(0);" onclick="submitAuditRevokeParole(\''
...
function viewProcess(url){
	//console.log(url);
	open_layer('撤销假释流程',url);
}
//流程审批
function submitAuditRevokeParole(taskId,id,auditType,processId){
	open_layer("撤销假释审核审批","${ctx}/jiangcheng/revokeParole/revokeParoleAudit?id=" + id+"&taskId="+taskId+"&auditType="+auditType+"&processInstanceId="+processId);
}
//详细
function revokeParole(id){
	var url = '${ctx}/jiangcheng/revokeParole/detail?id=' + id;
	open_layer('撤销假释详情',url);
}
//上报修改页面
function edit(id){
	open_layer('撤销假释（上报/修改）','${ctx}/jiangcheng/revokeParole/form?id='+id);
}
