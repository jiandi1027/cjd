...
function formatterAct(value,row,index){
	...
	if(row.finshInfo==0){
		...
		<shiro:hasPermission name="culpritinfo/culprit/update">
			operation=operation+space+'<a href="javascript:void(0);" class="organization-easyui-firstReport-pencil" data-options="plain:true,iconCls:\'fi-pencil icon-blue\'"  onclick="updateCulprit(\''+row.id+'\')">修改</a>';
		</shiro:hasPermission>
		/* if(row.dicFirstReportGetKey=='1570003'){//直接报到显示'修改'
			operation=operation+'<a href="javascript:void(0);" class="organization-easyui-firstReport-pencil" onclick="updateCulprit(\''+row.id+'\')">修改</a>'+space;
		}else{
			operation=operation+'<a href="javascript:void(0);" class="organization-easyui-firstReport-pencil1" onclick="updateCulprit(\''+row.id+'\')">处理</a>'+space;
		} */
	}
		...
		<table class="easyui-datagrid" id="culpritInfo_list_content" title="首次报到人员列表"  
			...
			<thead>
				<tr>
					...
					<th data-options="field:'dicSexKey',width:'5%',align:'center',formatter:function(value,row,index){
					...
					<th data-options="field:'dicFirstReportTypeKey',width:'5%',align:'center',formatter:function(value,row,index){
