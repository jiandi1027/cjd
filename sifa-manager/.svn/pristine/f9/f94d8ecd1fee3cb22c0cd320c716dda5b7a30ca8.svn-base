...
<div style="margin: 0 2px;padding: 0;font-family:FangSong_GB2312;">
	...
	<div id="sqjzxgsPrint">
		...
		width:165mm;height: 200mm;border:0;margin-right:auto;margin-left: auto;">
			...
			社区矫正人员<b>${culprit.name}</b>:
			</div>
			<div style="text-indent:28px;letter-spacing:3px;line-height:40px;margin:0;">
			你因<b>${culprit.culpritJudgment.crimeInfo}</b>罪经<b>${culprit.culpritJudgment.adjudgeOrg}</b>于
			<b><fmt:formatDate value="${culprit.culpritJudgment.adjudgeDate}" pattern="yyyy年MM月dd日" /></b>; 
			判处<b>${culprit.culpritJudgment.adjudgePeriod}（同时宣告禁止${culprit.culpritJudgment.timeBan}）</b>。
			<b><fmt:formatDate value="${culprit.culpritJudgment.adjudgeRedressT}" pattern="yyyy年MM月dd日"/></b>
			经<b>${culprit.culpritJudgment.adjudgeRedressOrg}</b>人民法院（监狱管理局、公安局）裁定假释（决定、批准暂予监外执行）。在管制（缓刑、假释、暂予监外执行）期间，依法实行社区矫正。
			社区矫正期限自<b><fmt:formatDate value="${culprit.culpritJudgment.redressStartDate}" pattern="yyyy年MM月dd日" /></b>起至<b><fmt:formatDate value="${culprit.culpritJudgment.redressEndDate}" pattern="yyyy年MM月dd日" /></b>止。
			...
			<div style="margin:0;text-indent:28px;line-height:40px;">
			四、司法所为你确立了社区矫正小组，小组成员由<b>${monitorGroup}</b>组成，协助对你进行监督、教育、帮助，你必须积极配合。
			</div>
			<div  style="margin:0;text-indent:28px;line-height:40px;">
			特此宣告。
			</div>
