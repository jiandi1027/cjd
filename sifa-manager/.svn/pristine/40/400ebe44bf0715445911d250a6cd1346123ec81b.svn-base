...
<%@ include file="/WEB-INF/views/include/taglib.jsp"%>
 ...
 <form  action="${ctx}/rewardspunishment/parole/submitParoleAudit" method="post" id="paroleAuditForm">
     ...
     <div class="easyui-panel" title="假释相关信息" style="width:100%;padding:5px;margin-bottom: 30px;">
   ...
   <!--  <div class="easyui-panel" id="p3" title="质量评定" style="width:100%;padding:10px;margin-bottom: 30px;">
    ...
    <c:if test="${parole.auditType eq 'sfsReport' }">
     ...
     <%--<div  style="text-align: center;display: none;">
         <p colspan="4">
         <c:if test="${parole.auditType eq 'jcyCheck'}">
         <a href="javascript:void(0)" class="easyui-linkbutton" id="procuratorial" onclick="$('#procuratorate').dialog('open')">报检察院</a>
		  			&nbsp;&nbsp;&nbsp; &nbsp;&nbsp;&nbsp; &nbsp;&nbsp;&nbsp;
		 </c:if> 
		 <c:if test="${parole.auditType eq 'gs'}">
         <a href="javascript:void(0)" class="easyui-linkbutton" id="parolePublic" onclick="$('#publicity').dialog('open')">公示结果</a>
		  			&nbsp;&nbsp;&nbsp; &nbsp;&nbsp;&nbsp; &nbsp;&nbsp;&nbsp;
		 </c:if>
		 <c:if test="${parole.auditType ne 'jcyCheck' && parole.auditType ne 'gs'}">
         <a href="javascript:void(0)" id="paroleAudit" class="easyui-linkbutton" onclick="check()">审核</a>
		  			&nbsp;&nbsp;&nbsp; &nbsp;&nbsp;&nbsp; &nbsp;&nbsp;&nbsp;
		  </c:if>
		  <a id="workerPrint1" class="easyui-linkbutton">关闭</a>
		  <a href="javascript:void(0)" class="easyui-linkbutton" id="paroleUpdate" onclick="updated()">修改</a>
		  			&nbsp;&nbsp;&nbsp; &nbsp;&nbsp;&nbsp; &nbsp;&nbsp;&nbsp;
		  </p>
    </div>
   
     </form>
    	<div id="check" title="处理" modal=true draggable=false  data-options="iconCls:'fi-clipboard-pencil icon-blue'"
		...
		<form id="checkForm" action="" method="post">
			...
			<table class="grid">
				...
				<c:if test="${parole.auditType eq 'jzkCheck'}">
			    <tr>
			    	<th height=30 align=left >是否推荐为典型案例：</th>
					<TD  width="35%">
						<input type="radio" name="recommendStatus" value="1"/>是&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
						<input type="radio" name="recommendStatus" value="2" checked="checked"/>否
					</TD>
			 	...
			 	<c:if test="${parole.auditType eq 'jzkCheck'}">
				<TR>	
					<th height=30 align=left >输入质量：</th>
					<TD width="35%">
						<input type="button"  value="评定" onclick="$('#dd').dialog('open')"/>
					</TD>
				</TR>
				<TR>	
					<th height=30 align=left >质量评分：</th>
					<TD width="35%">
						<input  class="easyui-validatebox" required=true style="width:50px"
							 missingMessage="质量评分不能为空!"  readonly="readonly" id="handleQualityGoal" name="optScore" />
					</TD>
				</TR>
				</c:if>
				<c:if test="${parole.auditType eq 'jzkCheck'||parole.auditType eq 'fzkCheck'}">
				<TR>	
					<th height=30 align=left >执法质量：</th>
					<TD width="35%">
				...
				<td><textArea class="f1 easyui-validatebox"
					data-options="multiline:true" name="opinion" required=true
					style="width: 240px; height: 80px" onkeydown='textarealength(this,300)' onKeyUp="textarealength(this,300)" maxlength="300"></textArea>
					<br>
				...
				<tr align="center">
					<td colspan="2"><a href="javascript:void(0)" id="btn1" class="easyui-linkbutton" data-options="iconCls:'fi-check icon-blue'" onclick="applyCheck('${parole.processInstanceId}')">确定</a>
						<a  id="btn2" data-options="iconCls:'fi-x icon-blue'" class="easyui-linkbutton">取消</a></td>
				</tr>
			</table>
		</form>
	</div>
    <div id="updated" title="假释信息修改" modal=true draggable=true
		class="easyui-dialog" closed=true style="padding:5px; top:250px; left:650px; width:400px;">
	...
	<div id="dd" class="easyui-dialog" title="输入质量评定明细" style="width:500px;height:400px;padding:10px; top:250px; left:650px;"   
...
<div id="publicity" title="公示结果处理" modal=true draggable=false   data-options="iconCls:'fi-clipboard-pencil icon-blue'"
		class="easyui-dialog" closed=true style="width: 400px;padding:10px; top:250px; left:650px;">
		<form id="publicityForm" action="" method="post">
			<input type="hidden" name="id" value="" />
			<table id="publicityAdd" class="grid">
				<tr>
			    	<TH height=40 width=100px; align=left >公示结果：</TH>
					<TD width=120px;>
						<input type="text" name="decideType" value="1" style="display:none"/>
						<input id="result1"type="radio" name="showResult" value="1" checked="checked"/>无异议&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;	
				...
				<tr align="center">
					<td colspan="6"><a id="publicityBtn" class="easyui-linkbutton" data-options="iconCls:'fi-check icon-blue'" onclick="applyPublicity('${parole.processInstanceId}')">确定</a>
						<a href="javascript:void(0)" id="btn2" class="easyui-linkbutton" data-options="iconCls:'fi-x icon-blue'" onclick=" $('#publicity').dialog('close');">取消</a></td>
				</tr>
			</table>
		</form>
	</div>
	<div id="procuratorate" title="检察院意见处理" modal=true draggable=false data-options="iconCls:'fi-clipboard-pencil icon-blue'"
		class="easyui-dialog" closed=true style="width: 500px;padding:5px; top:250px; left:650px;">
		<form id="procuratorateForm" action="" method="post">
			...
			<table class="grid">
			    ...
			    <tr>
					<th><span class="c-red">*</span>审批意见:</th>
					<td>
				...
				<tr align="center">
					<td colspan="2"><a id="btn3" class="easyui-linkbutton" data-options="iconCls:'fi-check icon-blue'" onclick="applyProcuratorate('${parole.processInstanceId}','${parole.id }')">确定</a>
						<a href="javascript:void(0)" id="btn2" data-options="iconCls:'fi-x icon-blue'" class="easyui-linkbutton" onclick=" $('#procuratorate').dialog('close');">取消</a></td>
				</tr>
				--%>
			</table>
		</form>
	</div>
<!-- <div id="tt" class="easyui-dialog" title="执法质量评定明细" style="width:800px;height:600px;padding:10px; top:100px; left:450px;"   
        data-options="iconCls:'icon-save',resizable:true,modal:true,closed: true" >
