...
<body class="easyui-layout" id="lay">
	...
	<div region="center" border="false">
		<table class="easyui-datagrid" id="t_list_content" title="特殊原因恢复矫正列表"
			style="height: 400px;"
			data-options="singleSelect:true,collapsible:true,striped:true,toolbar:toolbar,loadMsg: '数据正在加载,请耐心的等待...' ,pagination:true,pageSize: 10 ,pageList:[5,10,15,20,50],fitColumns:true,url:'${ctx}/endcorrect/redressResume/list',method:'post'">
			<thead>
				<tr>
					...
					<th data-options="field:'groupName',width:100,align:'center'"
						editor="text">司法所</th>
					<th data-options="field:'redressStartTime',width:100,align:'center'"
						>恢复时间</th>
...
<script type="text/javascript">
	...
	var toolbar = [ {
		...
		handler : function() {
			...
			open_layer('新增特殊原因恢复矫正','${ctx}/endcorrect/redressResume/add');
	...
	function viewProcess(url){
		open_layer('特殊原因恢复矫正流程',url);
	}
	function submitAudit(id,auditType){
		open_layer("特殊原因恢复矫正审核审批","${ctx}/endcorrect/redressResume/check?id=" + id+"&auditType="+auditType);
	}
	function detail(id){
		var url = '${ctx}/endcorrect/redressResume/get?id=' + id;
		open_layer('特殊原因恢复矫正流程详情',url);
		/* $.ajax({
			type : 'get',
