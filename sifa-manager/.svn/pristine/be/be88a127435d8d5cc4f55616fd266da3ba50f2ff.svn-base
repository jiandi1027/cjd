...
<body class="easyui-layout" id="lay">
	...
	<div region="center" border="false">
		<table class="easyui-datagrid" id="t_list_content" title="期满解娇列表"
			style="height: 400px;"
			data-options="singleSelect:true,collapsible:true,striped:true,toolbar:toolbar,loadMsg: '数据正在加载,请耐心的等待...' ,pagination:true,pageSize: 10 ,pageList:[5,10,15,20,50],fitColumns:true,url:'${ctx}/endcorrect/finishRedress/list',method:'post'">
			<thead>
				<tr>
					...
					<th data-options="field:'created',width:100,align:'center'"
						>提交时间</th>
					<th data-options="field:'dicRedressShowKey',width:230,formatter:function(value,rowData,rowIndex){
						return sy.getDicValue('SF_FINISH_REDRESS','DIC_REDRESS_SHOW_KEY',value,'')},align:'center'">矫正期间表现 </th>
					<th data-options="field:'dicPleaAttitudeKey',width:230,formatter:function(value,rowData,rowIndex){
...
<script type="text/javascript">
	...
	var toolbar = [ {
		...
		handler : function() {
			...
			open_layer('新增期满解娇','${ctx}/endcorrect/finishRedress/add');
	...
	function formatOper(val,row,index){  
		...
		if(row.hasPermission==1){
			operation=operation+'&nbsp;&nbsp;&nbsp;<a href="javascript:void(0);" onclick="submitAudit(\''+row.id+'\',\''+row.auditType+'\')">流程处理</a>';
		}
	...
	function viewProcess(url){
		open_layer('期满解娇流程',url);
	}
	function submitAudit(id,auditType){
		open_layer("期满解娇审核审批","${ctx}/endcorrect/finishRedress/check?id=" + id+"&auditType="+auditType);
	}
	function detail(id){
		var url = '${ctx}/endcorrect/finishRedress/get?id=' + id;
		open_layer('期满解娇流程详情',url);
		/* $.ajax({
			type : 'get',
