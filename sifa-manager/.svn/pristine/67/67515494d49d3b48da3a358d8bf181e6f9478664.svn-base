...
<body class="easyui-layout" id="lay">
	...
	<div region="center" border="false">
		<table class="easyui-datagrid" id="t_list_content" title="重新犯罪列表"
			style="height: 400px;"
			data-options="singleSelect:true,collapsible:true,striped:true,toolbar:toolbar,loadMsg: '数据正在加载,请耐心的等待...' ,pagination:true,pageSize: 10 ,pageList:[5,10,15,20,50],fitColumns:true,url:'${ctx}/endcorrect/recidivism/list',method:'post'">
			<thead>
				<tr>
					...
					<th data-options="field:'newCrime',width:100,align:'center'"
						editor="text">涉案罪名</th>
					<th data-options="field:'judgmentTime',width:100,align:'center'"
						>判决时间</th>
					<th data-options="field:'judgmentCourt',width:100,align:'center'"
						editor="text">判决单位</th>
					<th data-options="field:'registerTime',width:100,align:'center'"
						>立案时间</th>
...
<script type="text/javascript">
	...
	function formatOper(val,row,index){  
		...
		if(row.hasPermission==1){
			if(row.auditType=='workerReport'){
				operation=operation+'&nbsp;&nbsp;&nbsp;<a href="javascript:void(0);" onclick="add(\''+row.id+'\')">上报</a>';
			}else{
	...
	function viewProcess(url){
		open_layer('重新犯罪流程',url);
	}
	function add(id){
		open_layer("上报重新犯罪","${ctx}/endcorrect/recidivism/add?id=" + id);
	}
	function submitAudit(id){
		open_layer("重新犯罪审核审批","${ctx}/endcorrect/recidivism/check?id=" + id);
	}
	function detail(id){
		var url = '${ctx}/endcorrect/recidivism/get?id=' + id;
		open_layer('重新犯罪流程详情',url);
		/* $.ajax({
			type : 'get',
