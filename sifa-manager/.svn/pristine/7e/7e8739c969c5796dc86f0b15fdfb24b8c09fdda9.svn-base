...
<body class="easyui-layout" id="lay">
	...
	<div region="center" border="false">
		<table class="easyui-datagrid" id="t_list_content" title="涉嫌重新犯罪列表"
			style="height: 400px;"
			data-options="singleSelect:true,collapsible:true,striped:true,toolbar:toolbar,loadMsg: '数据正在加载,请耐心的等待...' ,pagination:true,pageSize: 10 ,pageList:[5,10,15,20,50],fitColumns:true,url:'${ctx}/endcorrect/recidivismPerhaps/list',method:'post'">
			<thead>
				<tr>
...
<script type="text/javascript">
	...
	var toolbar = [ {
		...
		handler : function() {
			...
			open_layer('新增涉嫌重新犯罪','${ctx}/endcorrect/recidivismPerhaps/add');
	...
	function viewProcess(url){
		open_layer('涉嫌重新犯罪流程',url);
	}
	function submitAudit(id,auditType){
		open_layer("涉嫌重新犯罪审核审批","${ctx}/endcorrect/recidivismPerhaps/check?id=" + id+"&auditType="+auditType);
	}
	function detail(id){
		var url = '${ctx}/endcorrect/recidivismPerhaps/get?id=' + id;
		open_layer('涉嫌重新犯罪流程详情',url);
		/* $.ajax({
			type : 'get',
