	        ...
	        <form id="searchForm"  style="padding-bottom: 10px;padding-top: 10px;">
				...
				<table>
					...
					<tr>
						...
						onfocus="var searchEndDate_reduce=$dp.$('searchEndDate_reduce')+1;WdatePicker({skin:'blueFresh',dateFmt:'yyyy-MM-dd HH:mm',maxDate:'#F{$dp.$D(\'searchEndDate_reduce\')}'})"/>
						</td>
					</tr>
					<tr>
						<td class="search-title">申请截止：</td>
						<td><input name="searchEndDate" id="searchEndDate_reduce" class="Wdate easyui-validatebox"
						onfocus="WdatePicker({skin:'blueFresh',dateFmt:'yyyy-MM-dd HH:mm',minDate:'#F{$dp.$D(\'searchStartDate_reduce\')}'})"/></td>
					</tr>
				</table>
			</form>
			</div>
		...
		if(isEnd==0){
			 ...
			 parent.$.modalDialog({
		         ...
		         buttons : [ {
		             ...
		             handler : function() {
		                 var f = parent.$.modalDialog.handler.find('#reducePenaltyPrintBtn');
		                 f.click();
		             }
		         },{
		 				text : '关闭',
