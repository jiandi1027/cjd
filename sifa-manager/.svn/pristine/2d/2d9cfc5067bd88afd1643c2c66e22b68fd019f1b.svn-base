...
<body class="easyui-layout" id="lay">
	...
	<div region="center" border="false">
		<table class="easyui-datagrid" id="listContent" title="角色列表" style="height: 400px;"
			data-options="singleSelect:true,collapsible:true,striped:true,toolbar:toolbar,loadMsg: '数据正在加载,请耐心的等待...' ,pagination:true,pageSize: 10 ,pageList:[5,10,15,20,50],fitColumns:true,url:'${ctx}/sys/sysRole/list',method:'post'">
		<thead>
			<tr>
				...
				<th data-options="field:'available',width:80,align:'center',formatter:formatAvailable"   editor="{type:'validatebox',options:{required:true}}">是否可用</th>
				<th data-options="field:'created',width:250,align:'center'" editor="text">创建日期</th>
				<th data-options="field:'updated',width:250,align:'center'" editor="text">修改日期</th>
			</tr>
		</thead>
	</table>
	</div>
	...
	<div id="mydialog" title="新增角色" modal=true draggable=false
			...
			<table>
				...
				<tr>
					...
					<td><input type="text" name="ename"
						class="easyui-validatebox" required=true
						 missingMessage="英文名称必填!"
						 value="" /></td>
				</tr>
				<tr>
					<td>是否可用:</td>
					<td><input name="available" class="easyui-combobox"
						url="${ctx}/sys/sysDic/findSysDicByTabNameAndColumnName?tabName=common_table&columnName=yes_no&notNeParentId=0" valueField="value" textField="key"
						value="" /> </td>
				</tr>
		...
		if (arr.length <= 0) {
			...
			$.messager.confirm('提示信息', '确认删除?', function(r) {
				if (r) {
					...
					$.post('${ctx}/sys/sysRole/batchDelete', ids, function(result) {
						//1 刷新数据表格 
