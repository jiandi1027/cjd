...
<form id="revokeParoleAuditForm" name="revokeParoleAuditForm" action="${ctx}/rewardspunishment/revokeParole/submitRevokeParoleAudit" method="post">
	...
	<div class="easyui-panel" title="具体信息" style="width:100%;padding:5px;magin:0 auto;">
            <table width="100%" class="grid">
            	<tr>
                    <th width="10%">违纪原因：</th><td>${fns:getSysDicValueByKey('sf_revoke_probation','REASON_TYPE_ID',revokeParole.dicReasonTypeKey,'')}</td>
                </tr>
                 <tr>
					<th>结案报告：</th>
					<td>
                ...
                <tr>
					...
					<td>
						<textarea readOnly=true style="width:80%;height:80px;border: 0 none;">${revokeParole.detailInfo }</textarea>
					</td>
                </tr>  
                <tr>
                    <th width="10%">处理依据：</th>
               		<td>
						<textarea readOnly=true style="width:80%;height:80px;border: 0 none;">${revokeParole.fact }</textarea>
					</td>	
                </tr>
                <c:if test="${revokeParole.auditType eq 'sfsReport' || revokeParole.auditType eq 'jzkReport'}">
		             <tr>
		                 ...
		                 <td> 
		                 	<a onclick="printRevokeParoleBook('${revokeParole.id}');" href="javascript:void(0);" class="easyui-linkbutton" data-options="plain:true,iconCls:'fi-pencil icon-blue'">编辑</a>
		                 </td>
		             </tr>
				 </c:if>
               <!--  <tr>
...
<div class="easyui-panel" title="审批信息" style="width:100%;padding:5px;magin:0 auto;">
            <table width="100%" class="grid">
            	<TR>	
					<Th width="10%" >审核结果：</Th>
					<TD  width="23%">
				...
				<TR>	
					...
					<TD width="23%">
						<select style="width:150px;" editable=false name="submittionOrgTypeId" required=true missingMessage="呈报不能为空!" value="${revokeParole.submittionOrgTypeId }" panelHeight=100
  ...
  <div id="revokeParolerewardspunishmentForm"></div>
<script type="text/javascript">
var adtype = '${revokeParole.auditType}';
//编辑撤销假释建议表
function printRevokeParoleBook(id){
	 $('#printJiashi').dialog({    
		    title: '编辑《撤销假释建议书》',    
		    width: 900,    
		    height: 800,    
		    ...
		    href: '${ctx}/rewardspunishment/revokeParole/preSubmit?id='+id,    
		    modal: true,
		    buttons :[{
				text:'保存',
	            iconCls:'fi-print icon-blue',
				handler:function(){
					$('#saveParoleBook').click();
				}
			}, {
				text:'关闭',
...
var result='';
 $(function() {
	...
	$('#revokeParoleAuditForm').form({
		url:'${ctx}/rewardspunishment/revokeParole/submitRevokeParoleAudit',
		onSubmit:function(){
			 progressLoad();
			 var adtypemsg = '';
		...
		success:function(data){
			...
			if(data.status==200){
				...
				parent.$.modalDialog.openner_dataGrid.datagrid('reload');
                 ...
                 $('#revokeParolerewardspunishmentForm').dialog('destroy');
			}else{
...
//评分
 function rewardspunishment(type){
 		var url = '${ctx}/sys/mxCheck/assess?infoType='+ type +'&processInstanceId=${revokeParole.processInstanceId}&tablename=revokeParolerewardspunishmentForm';
 		var title = "评定明细";
 		...
 		$('#revokeParolerewardspunishmentForm').dialog({    
 		    title: title,    
 		    width: 600,
 		    height: 520,
 		    href: url,    
 		...
 		$('#revokeParolerewardspunishmentForm').window('center');
 	}
 	...
 	$('#revokeParolerewardspunishmentForm').dialog({    
	    ...
	    iconCls:'fi-checkbox icon-blue',
 	    buttons :[{
 			...
 			iconCls:'fi-x icon-blue',
 			handler:function(){
 				$('#revokeParolerewardspunishmentForm').dialog('close');
 			}
 		}]
 	});
 	$('#revokeParolerewardspunishmentForm').window('center');
