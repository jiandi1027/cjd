...
<body class="easyui-layout" id="lay">
    ...
    data-options="url:'${ctx}/sys/sysGroup/loadGroupTree',editable:true">
					...
					<td width="15%" align="right">劳动内容</td>
					<td>
						<input name="workContent" class="easyui-textbox"
						 style="width: 80%;" />
					</td>
				</tr>
	...
	<div region="center" border="false">
		<table class="easyui-datagrid" id="t_list_content" title="社区服务列表"
			style="height: 400px;"
			data-options="singleSelect:true,collapsible:true,striped:true,toolbar:toolbar,loadMsg: '数据正在加载,请耐心的等待...' ,pagination:true,pageSize: 10 ,pageList:[5,10,15,20,50],fitColumns:true,url:'${ctx}/dailymgr/workfree/list',method:'post'">
			<thead>
				<tr>
					<th data-options="field:'groupName',width:100,align:'center'"
						editor="text">组织单位</th>
					<th data-options="field:'workContent',width:100,align:'center'"
						editor="text">劳动内容</th>
					<th data-options="field:'workDate',width:100,align:'center'"
						>劳动日期</th>
					<th data-options="field:'workPlace',width:100,align:'center'"
						editor="text">劳动地点</th>
...
<script type="text/javascript">
	...
	var toolbar = [ {
		...
		handler : function() {
			...
			open_layer('新增社区服务','${ctx}/dailymgr/workfree/form');
	...
	function detail(id){
		var url = '${ctx}/dailymgr/workfree/get?id=' + id;
		open_layer('社区服务详情',url);
	}
	
	function update(id){
		var url = '${ctx}/dailymgr/workfree/form?id=' + id;
		open_layer('更新社区服务',url);
	}
