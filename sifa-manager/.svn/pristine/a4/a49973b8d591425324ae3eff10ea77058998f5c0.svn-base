...
sy.getDicValue = function(tabName,columnName,key,defaultValue){
	...
	$.ajax({
		type : 'post',
		url : sy.pn()+'/sys/sysDic/getDicValue?tabName='+tabName+'&columnName='+columnName+'&key='+key+"&defaultValue="+defaultValue,
		cache : false,
		async : false, //同步请求
		data : {
			id : key
 ...
 * 获取数据字典值
 */
sy.getSysDicList = function(tabName,columnName,key){
	var str = '';
	$.ajax({
		type : 'post',
		url : sy.pn()+'/sys/sysDic/getListByTabColumnName?tabName='+tabName+'&columnName='+columnName+'&key='+key,
		cache : false,
		async : false, //同步请求
		data : {
			id : key
