...
<script type="text/javascript">
	...
	function readNode(node) {
		...
		var title=node.text;
		if(title.indexOf('workRemindOfIndex')>0){
			title="待处理事项";
		}
		if(title.indexOf('examAndAppOfIndex')>0){
			title="待审批事项";
		}
		var opts = {
			title : title,
	...
	<div id="index_layout">
		...
		<div data-options="region:'west',split:true" title="菜单"
			...
			<c:if test="${(fns:getSysAccount()).roleId != 'kqxt'}">
				...
				<div title="待办事项(${allAmountHomePage })" iconCls="fi-lightbulb">
					<ul class="easyui-tree tree" style="border: left"
						data-options="fit:true,border:false,onClick:readNode">
						<li iconCls="fi-compass icon-blue"
							data-options="attributes:{'url':'${ctx}/homepage/examinationAndApproval/list'}">待审批事项
							(<div style="color:red;display:inline" title="abcd" id="examAndAppOfIndex">${allAmountExamAndApp }</div>)
						</li>
						<li iconCls="fi-clock icon-blue" 
							data-options="attributes:{'url':'${ctx}/homepage/workRemind/list'}">待处理事项
							(<div style="color:red;display:inline" id="workRemindOfIndex">${allAmountWorkRemind }</div>)
						</li>
					</ul>
				</div>
				<div title="调查评估" iconCls="fi-page-csv">
					<ul class="easyui-tree tree" style="border: left"
						data-options="fit:true,border:false,onClick:readNode">
						<li iconCls="fi-magnifying-glass icon-blue"
							data-options="attributes:{'url':'${ctx}/investigate/list'}">调查评估</li>
					</ul>
				</div>
				...
				<div title="监控管理" iconCls="fi-map">
					<ul class="easyui-tree tree" style="border: left"
						...
						<li iconCls="fi-alert icon-blue"
							data-options="attributes:{'url':'${ctx}/monitoring/overAlarm/list'}">越界报警</li>
						<li iconCls="fi-alert icon-blue"
							data-options="attributes:{'url':'${ctx}/monitoring/removeAlarm/list'}">拆卸报警</li>
