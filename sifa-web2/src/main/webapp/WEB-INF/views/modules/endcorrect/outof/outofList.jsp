	...
	<div data-options="region:'east',border:true,split:false,title:'查询条件'" style="width: 240px;">
		<div class="easyui-layout" data-options="fit:true,border:false">
			<c:if test="${!fns:isLeafGroup(fns:getSysAccount().groupId) }">
				<sys:sysGroup hiddenId="groupId" hiddenName="groupId" dataGridId="t_list_outof" />
			</c:if>
		<div class="easyui-panel search-form"
			data-options="fit:true,border:false" id="p2" title="<c:if test="${!fns:isLeafGroup(fns:getSysAccount().groupId) }">查询项目</c:if>"
			style="width: 100%;">
			<table>
				<tr>
		...
		<div data-options="region:'south',border:false"
			...
			<a href="#" class="easyui-linkbutton" id="outofSearchbtn" iconCls="icon-search">查询</a> &nbsp;&nbsp; 
		...
		<table class="easyui-datagrid" id="t_list_outof" title="迁出列表"
			...
			<thead>
				<tr>
					...
					<th data-options="field:'culpritName',width:'11%',align:'center'"
						editor="{type:'validatebox',options:{required:true}}">矫正对象</th>
					<th data-options="field:'groupName',align:'center'" style="width:11%;">司法所</th>
					<th data-options="field:'place',width:'18%',align:'center'"
						editor="{type:'numberbox',options:{required:true}}">迁出地</th>
					<th data-options="field:'moveTime',width:'11%',align:'center'">迁出日期</th>
					<th data-options="field:'isPrivate',width:'7%',align:'center',formatter:formatIsPrivate">是否私自</th>
