...
<div class="easyui-layout" id="layOut20" data-options="fit:true">
	<div region="north" border="false" title="查询条件 " style="height:65px;" data-options="collapsed:true">
		<form id="workRemindSearchForm20">
			<table width="100%" class="grid">
				<tr>
					...
					<td class="search-title" style="text-align:left">
						 <a href="#" class="easyui-linkbutton" iconCls="icon-search" onclick="javascript:$('#view_list_workRemind20').datagrid('load' ,$.serializeObject($('#workRemindSearchForm20')));">查询</a> &nbsp;&nbsp; 
						<a href="#" class="easyui-linkbutton"  iconCls="icon-undo" onclick="javascript:$('#layOut20').layout('collapse','north');$('#workRemindSearchForm20').form('clear');$('#view_list_workRemind20').datagrid('load', {});">取消</a></div>
					</td>
				</tr>
			</table>
		</form>
	...
	<div region="center" border="false">
		<table class="easyui-datagrid" id="view_list_workRemind20" title="待办事项列表"
			data-options="singleSelect:true,fit:true,rownumbers:true,toolbar:[{text:'查询',iconCls:'icon-search',handler:function(){$('#layOut20').layout('expand','north');}}],fitColumns:true,collapsible:true,striped:true,
			pagination:true,pageSize: 15,pageList:[5,10,15,20,50],loadMsg: '数据正在加载,请耐心的等待...' ,url:'${ctx}/homepage/workRemind/toViewList?key=${key}&groupId=${groupId}',method:'post',onLoadSuccess:function (data) {
				   ...
				   $('.organization-easyui-workRemind-jilu').linkbutton({text:'报到记录',plain:true,iconCls:'fi-list icon-blue'}); 
				   }">
				  
			<thead>
			<tr>
					<th data-options="field:'id',checkbox:true">编号</th>
					<th data-options="field:'culpritName',width:'25%',align:'center'">姓名</th>
					...
					<th data-options="field:'action',width:'23%',align:'center'" formatter="formatterAct">操作</th>
				</tr>
			</thead>
		</table>
	</div>
...
function formatterAct(val,row,index){
	var space = '&nbsp;';
	var operation='<a href="javascript:void(0);" class="organization-easyui-workRemind-jilu" onclick="detailWorkRemind(\''+row.culpritId+'\')"></a>';
