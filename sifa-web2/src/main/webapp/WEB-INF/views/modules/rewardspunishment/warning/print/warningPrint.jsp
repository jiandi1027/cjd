...
<form action="" method="post" id="">
	...
	<div id="warning_PrintMsg2"></div>
	<div class="easyui-panel" id="culpritWarningPrint" style="width:100%;border:0" align="center">
		...
		<table width="100%" style="border-collapse:collapse;border-spacing:0;border-left:1px solid;border-top:1px solid;text-align: center;width:680px;">
			...
			<tr style="height: 50px;">
				...
				<td style="border-right:1px solid;border-bottom:1px solid;">${fns:getSysDicValueByKey('sf_investigate','entrust_type_id',securityPunishment.culpritJudgment.dicPenalTypeKey,'')}</td>
				<td style="border-right:1px solid;border-bottom:1px solid;">矫正期限</td>
				<td style="border-right:1px solid;border-bottom:1px solid;">${securityPunishment.culpritJudgment.redressPeriod}</td>
			...
			<tr style="height: 100px;">
				...
				<td style="border-right:1px solid;border-bottom:1px solid;" colspan="5">
					<span style="display: block;line-height: 100px;vertical-align:middle;">
						<c:if test="${ not empty warning.fact}">
							主要事实：${warning.fact}<br>
						</c:if>
						<c:if test="${not empty warning.legalBasis || not empty warning.notePathName || not empty warning.legalBasisItems}">
							依据：
							<c:if test="${not empty warning.legalBasis }">
								${warning.legalBasis}；
							</c:if>
							<c:if test="${not empty warning.notePathName }">
								${warning.notePathName}；
							</c:if>
							<c:if test="${not empty warning.legalBasisItems }">
			...
			<tr style="display: none;">
				<td colspan="100">
					<a id="warningPrintBtn2" class="easyui-linkbutton">打印</a>
...
$('#warningPrintBtn2').click(function (){
	CreatePrintPage();
	LODOP.PREVIEW();
});
...
function CreatePrintPage(){
	LODOP = getLodop(document.getElementById('LODOP_OB'),document.getElementById('LODOP_EM'),document.getElementById('warning_PrintMsg2'));
	LODOP.PRINT_INIT("打印社区服刑人员警告审批表");
    //设置打印纸张大小，1代表纵向打印
    LODOP.SET_PRINT_PAGESIZE(1, 0, 0, "A4");
    LODOP.ADD_PRINT_HTM(40,40,"60%","100%",$('#culpritWarningPrint').html());
