...
<form id="revokeProbationAuditForm" name="revokeProbationAuditForm" action="${ctx}/rewardspunishment/revokeProbation/submitRevokeProbationAudit" method="post">
	...
	<c:choose>
	 ...
	 <div class="easyui-panel" title="具体信息" style="width:100%;padding:5px;">
            <table width="100%" class="grid">
                ...
                <tr>
                    <th>简要违纪情况：</th>
					<td>
						<textarea readOnly=true style="width:80%;height:80px;border: 0 none;">${revokeProbation.detailInfo }</textarea>
					</td>
                </tr>  
                <tr>
                    <th>处理依据：</th>
               		<td>
						<textarea readOnly=true style="width:80%;height:80px;border: 0 none;">${revokeProbation.fact }</textarea>
					</td>	
                </tr>
                <c:if test="${revokeProbation.auditType eq 'sfsReport' || revokeProbation.auditType eq 'jzkReport'}">
		             <tr>
		                 <th>编辑建议书：</th>
		                 <td> 
		                 	<a onclick="printRevokeProbationBook('${revokeProbation.id}');" href="javascript:void(0);" class="easyui-linkbutton" data-options="plain:true,iconCls:'fi-pencil icon-blue'">编辑</a>
		                 </td>
		             </tr>
				 </c:if>
                <!-- <tr>
...
<c:if test="${revokeProbation.auditType ne 'sfsReport' && revokeProbation.auditType ne 'jzkReport'
 ...
 <div class="easyui-panel" title="审批信息" style="width:100%;padding:5px;">
            <table width="100%" class="grid">
				...
				<TR>	
					...
					<TD width="23%">
						<select style="width:150px;" editable=false name="submittionOrgTypeId" required=true missingMessage="呈报不能为空!" value="${revokeProbation.submittionOrgTypeId }" panelHeight=100
 ...
 <div id="revokeProbationerewardspunishmentForm"></div>

<script type="text/javascript">
var adtype = '${revokeProbation.auditType}';
//编辑撤销缓刑建议表
function printRevokeProbationBook(id){
	 $('#printBianJi').dialog({    
		    title: '编辑《撤销缓刑建议书》',    
		    width: 900,    
		    height: 700,    
		    ...
		    href: '${ctx}/rewardspunishment/revokeProbation/preSubmit?id='+id+'&audit=1',    
		    modal: true,
		    buttons :[{
				text:'保存',
...
var result='';
 $(function() {
	...
	$('#revokeProbationAuditForm').form({
		url:'${ctx}/rewardspunishment/revokeProbation/submitRevokeProbationAudit',
		onSubmit:function(){
			 progressLoad();
			 var adtypemsg = '';
...
function rewardspunishment(type){
		var url = '${ctx}/sys/mxCheck/assess?infoType='+ type +'&processInstanceId=${revokeProbation.processInstanceId}&tablename=revokeProbationerewardspunishmentForm';
		var title = "评定明细";
		...
		$('#revokeProbationerewardspunishmentForm').dialog({    
		    title: title,    
		    width: 600,
		    height: 520,
		    href: url,    
		...
		$('#revokeProbationerewardspunishmentForm').window('center');
	}
...
function mxcheckdetail(infotype){
	...
	$('#revokeProbationerewardspunishmentForm').dialog({    
	    ...
	    buttons :[{
			...
			iconCls:'fi-x icon-blue',
			handler:function(){
				$('#revokeProbationerewardspunishmentForm').dialog('close');
			}
		}]
	});
	$('#revokeProbationerewardspunishmentForm').window('center');
