...
var LODOP; //声明为全局变量 
//打印当前页
 function preview_penalTyleBook(){
	 CreatePrintPage();
	 LODOP.PREVIEW();
} 
...
function CreatePrintPage(){
	LODOP = getLodop(document.getElementById('LODOP_OB'),document.getElementById('LODOP_EM'),document.getElementById('crime_PrintMsg'));
    LODOP.PRINT_INIT("矫正对象刑罚类型比例分析表");
    //设置打印纸张大小，1代表纵向打印
    LODOP.SET_PRINT_PAGESIZE(1, 0, 0, "A4");
    LODOP.ADD_PRINT_HTM(40,60,"70%","100%",$('#penalTypePrint').html());
}
</script>
	<div>
		<div style="text-align:center;margin-top:10px;">
			<a href="#" class="easyui-linkbutton" data-options="iconCls:'fi-print icon-blue'" id="crimePrintBtn" onclick="preview_penalTyleBook()">打印当前页</a>  
		</div>
		<div id="penalType_PrintMsg"></div>
		<div class="easyui-panel" id="penalTypePrint" style="width:100%;border:0" align="center">
			...
			<div style="text-align: center;margin-bottom:40px;width:670px;">
				<span style="font-weight: bold;font-size: 24px;">矫正对象刑罚类型比例明细表</span>
			</div>
			...
			<table width="100%" style="margin-top:10px; border-collapse:collapse;border:1px solid;text-align: left;width:670px;">
				...
				<c:forEach items="${penalTypeMap}" var="penalTypes" varStatus="status">
              		<tr>                                                                                                                                                                                                                                                                                       	
                		<td style="height:50px;text-align: center;border-right: 1px solid;border-bottom: 1px solid;">${status.count}</td>
                		<td style="height:50px;text-align: center;border-right: 1px solid;border-bottom: 1px solid;">${penalTypes.TYPENAME}</td>
                		<td style="height:50px;text-align: center;border-right: 1px solid;border-bottom: 1px solid;">${penalTypes.COUNT}</td>
                		<td style="height:50px;text-align: center;border-right: 1px solid;border-bottom: 1px solid;">${penalTypes.PERCENT}</td>
					</tr>
              	</c:forEach>
              	<tr>
