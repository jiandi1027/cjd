...
<object  id="LODOP_OB" classid="clsid:2105C259-1E0C-4534-8141-A753534CB4CA" width=0 height=0> 
	...
	<div>
		<div style="display:none">
			<a href="#" class="easyui-linkbutton" data-options="iconCls:'fi-print icon-blue'" id="printRevokeParoleBook" onclick="preview_revokeParole()">打印当前页</a> 
		</div>
		<div id="revokeParole_PrintMsg"></div>
	    <div id="revokeParolePrint">
		...
		margin-top:40px;width:165mm;height: 200mm;border:0;margin-right:auto;margin-left: auto;">
			...
			<div style="margin-top:3px;text-indent:28px;line-height:30px;padding:0;margin: 0;">
				...
				<u><fmt:formatDate value="${culprit.birthday}" pattern="yyyy年MM月dd日" /></u>出生，
				 居住地：<u>${culprit.address }</u>，户籍地：<u>${culprit.regResidence }</u>。
			...
			<div style="margin-top:3px;line-height:30px;padding:0;margin: 0;" >
				...
				style="text-indent:28px;border:0 none;overflow:hidden;font-size:19px;text-align:left;width:620px;height:300px;font-family: FangSong_GB2312;">${revokeParole.concreteFacts}</textarea>
			</div>
			<div style="font-size:19px;;margin-top:3px;text-indent:28px;line-height:30px;">
				根据《中华人民共和国刑法》第七十七条、《社区矫正实施办法》第二十五条、《广西壮族自治区社区矫正实施细则（试行）》第八十七条之规定，建议对社区矫正人员<u>${culprit.name}</u>撤销假释。
			</div>
			...
			<div style="font-size:19px;margin-top:30px;line-height:30px;">
				抄送：<u>${revokeParole.procuratorate}</u>人民检察院，<u>${revokeParole.police}</u>公安（分）局。
			</div>
		</div>
		</div>
</div>	
...
function CreatePrintPage(){
	LODOP = getLodop(document.getElementById('LODOP_OB'),document.getElementById('LODOP_EM'),document.getElementById('revokeParole_PrintMsg'));
  LODOP.PRINT_INIT("打印撤销缓刑建议书");
  //设置打印纸张大小，1代表纵向打印
  LODOP.SET_PRINT_PAGESIZE(1, 0, 0, "A4");
  LODOP.ADD_PRINT_HTM(40,70,"70%","100%",$('#revokeParolePrint').html());
