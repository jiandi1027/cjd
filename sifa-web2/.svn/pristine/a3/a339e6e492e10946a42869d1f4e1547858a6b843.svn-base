...
<%@ include file="/WEB-INF/views/include/taglib.jsp"%>
     <div class="easyui-panel" title="户籍类型分析" style="width:100%;padding:5px;height:100%;">
		<div id="hjlx" data-options="fit:'true'" style="width: 90%; height: 90%;"></div>
    </div>
    <script type="text/javascript"> 
    $.ajax({
        type: "post",
        url: "${ctx}/statistics/chart/getHjlx?groupId=${groupId}&dicStatusKey=${dicStatusKey}&searchStartDate=${searchStartDate}&searchEndDate=${searchEndDate}",
        dataType: "json",
        async: false,
        success: function(result){
            ...
            var criChart = echarts.init(document.getElementById('hjlx'));
              // 指定图表的配置项和数据
            option = {
          title : {
              text: '矫正对象户籍比例分析图',//主标题
              x:'center',
              textStyle: {
