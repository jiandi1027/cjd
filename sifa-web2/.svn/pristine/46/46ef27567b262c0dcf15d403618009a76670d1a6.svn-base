...
<div class="easyui-panel" style="width:100%;padding:5px;margin:0 auto;">
		<form id="auditWorkRemindForm10" action="" method="post">
...
<div id="dealAdd10"></div>
<script>
$('#auditWorkRemindBtn').click(function(){
	if($('#auditWorkRemindForm10').form('validate')){
		var dialogParent = $('#dealAdd10').parent();  
	    var dialogOwn = $('#dealAdd10').clone();  
	    dialogOwn.hide();
		        ...
		        buttons : [ {
		            ...
		            handler : function() {
		                ...
		                $.ajax({
							...
							cache:false ,
							data: $.serializeObject($('#auditWorkRemindForm10')),
							dataType:'json',
							success:function(result){
								$('#dealAdd10').dialog('close');
								$('#audit10').dialog('close');
						   		$('#view_list_workRemind10').datagrid('reload');
							} 
						});
		            }
		        },{
