 ...
 <form  action="${ctx}/dailymgr/gradeChange/save" method="post" id="gradeChangeInfo">
	...
	<div class="easyui-panel" title="相关信息" style="width:100%;padding:5px;margin-bottom: 30px;">
		...
		<table width="100%" class="grid">
			<tr>
			    ...
			    <td width="23%" style="border-right:none">${fns:getSysDicValueByKey('sf_culprit','dic_gldxjb_key',culprit.dicMonitorGradeKey ,'')} </td>
			    <td width="10%" style="border-left:none;border-right:none;"></td>
			    <td width="57%" style="border-left:none;"></td>
			</tr>
	        <tr>
	        	<th width="10%"><span class="c-red">*</span>新监管等级：</th>
	            <td width="23%">
	            	<input name="dicMonitorGradeKey" id="dicMonitorGradeKey" class="easyui-combobox" required=true  style="width:200px;"
	            		data-options="valueField:'id',textField:'value',url:'${ctx}/sys/sysDic/getDicOnlyByParent?parentKey=27000&key=${culprit.dicMonitorGradeKey }',panelHeight:'auto',editable:false"></input>
	            </td>
	            <th width="10%"><span class="c-red">*</span>确定监管等级缘由：</th>
	            <td width="57%">
                   	<input id="dicChangeReasonKey" name="dicChangeReasonKey" class="easyui-combobox" required=true style="width:300px;"
                   		data-options="panelMaxHeight:'200',panelHeight:'auto',limitToList:'true'">
	            </td>
	        </tr>
			<tr >
			...
			<tr>
				<th>相关材料：</th>
				<td colspan="3"><div id="evidencePath" ></div></td>
			</tr>
		</table>           
    </div>
</form>
    ...
    $('#dicMonitorGradeKey').combobox({
    	onSelect: function(record){
        	...
        	if( record.id == "27003") {
        		ChangeReasonKey = 121000 ;
            }
        	else if( record.id == "27004" ) {
        		ChangeReasonKey = 119000 ;
            }
        	else if( record.id == "27005" ) {
        		ChangeReasonKey = 120000 ;
            }
    		$('#dicChangeReasonKey').combobox({    
    			url:'${ctx}/sys/sysDic/getDicOnlyByParent?parentKey=' + ChangeReasonKey ,
    		    valueField:'id',    
    		    textField:'value'
    		}); 
    	}
